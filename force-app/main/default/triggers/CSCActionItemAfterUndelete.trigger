trigger CSCActionItemAfterUndelete on CSC_Action_Item__c (after undelete) {
    ...
    /*[04/03/14] - T-268092 - RA - This custom setting is used to conditionally control for which users/profile or the entire org
      the triggers, validation rules and workflow rules fire*/
        List<CSC_Action_Item__c> actionItems = [SELECT Id, Order__c FROM CSC_Action_Item__c WHERE Id IN :trigger.newMap.keySet()];

        for(CSC_Action_Item__c actionItem : actionItems) {
            actionItem.Order__c = CSCUtils.ActionListOrderThreshold;
        }
